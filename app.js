...
function multiplyAnyArray(testDynamicArray) { //eslint-disable-line
  ...
  for (var i=1; i < testDynamicArray.length ; i++){
    var letsGo = multiply(testDynamicArray[0], testDynamicArray[i]);
    storedNumber *= letsGo[0];
    var talk= 'The numbers ' + testDynamicArray + ' have a product of ' + storedNumber + '.';
  }
  return [storedNumber, talk];
}
